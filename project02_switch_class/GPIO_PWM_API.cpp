...
// ====================  GPIO_SWITCH ========================
// 생성자
GPIO_SWITCH::GPIO_SWITCH(int pin) {
    mPinNum = pin;
    wiringPiSetupGpio();
    pinMode(pin, INPUT);
}

GPIO_SWITCH::GPIO_SWITCH(int pin, int edge) {
    mPinNum = pin;
    wiringPiSetupGpio();
    pinMode(pin, INPUT);
}

GPIO_SWITCH::GPIO_SWITCH(int pin, int edge, void (*func)(void)) {
    mPinNum = pin;
    wiringPiSetupGpio();
    pinMode(pin, INPUT);
    // 2: RISING_MODE, 1: FALLING
    wiringPiISR(mPinNum, edge, func);
}

// 멤버 함수
int GPIO_SWITCH::switchRead() {
    return digitalRead(mPinNum);
}

int GPIO_SWITCH::switchModeRead(void) {
    return mMode;
}
