...
int main(int argc, char *argv[])
{
    ...
    pW->PIR = new GPIO_PIR(pirPin);
    pthread_t pir_t_id;
    if(pthread_create(&pir_t_id, NULL, pirThread, NULL) != 0)
        qDebug("Thread create error!");
...
void* pirThread(void *arg)
{
    while (1)
    {
        if(pW->PIR->readData())
        {
            pW->ui->PirSensor->setEnabled(true);
            delay(2000);
        }
        else
            pW->ui->PirSensor->setEnabled(false);
    }
}
